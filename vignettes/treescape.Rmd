...
```{r treescape, fig.width=3, fig.height=3}
...
```{r treescapescatter, fig.width=5, fig.height=5}
scatter(res$pco)
```

We illustrate this with *ape*'s dataset *woodmouse*, from which we built the 201 trees supplied in __`woodmiceTrees`__ using the neighbour-joining and bootstrapping example from the *ape* documentation. Here we plot the first 2 PCs  with *ggplot2*.
```{r woodmice, fig.width=8, fig.height=8}
data(woodmiceTrees)
...
```{r woodmiceMedian, fig.width=5, fig.height=5}
...
```{r woodmice1med, fig.width=5, fig.height=5}
...
```{r woodmiceCluster2, fig.width=5, fig.height=5}
